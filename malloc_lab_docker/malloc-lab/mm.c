...
{
    size_t prev_alloc = GET_ALLOC(FTRP(PREV_BLKP(bp))); //이전 블록의 alloc 값(할당 여부)를 prev_alloc에 대입한다.
    ...
    else if(prev_alloc && !next_alloc)                  //next_alloc이 할당되지 않았다면
    {
        ...
        PUT(HDRP(bp), PACK(size, 0));                   //PACK(size와 0을 받아서 or연산을 한다.) 해당 값을 현재 bp의 헤더가 가리키는 워드에 저장한다. 
        PUT(FTRP(bp), PACK(size, 0));                   //PACK한 값을 현재 bp의 풋터(FTRP 메크로에서 확장된 블록 사이즈로 풋터를 구한다.) 해당 값을 확장된 bp의 풋터가 가리키는 워드에 저장
    }

    else if(!prev_alloc && next_alloc)                  //prev_alloc이 할당되지 않았다면
    {
        size += GET_SIZE(HDRP(PREV_BLKP(bp)));          //size(현재 블록의 사이즈에)에 이전 블록의 size를 더한다.
        PUT(FTRP(bp), PACK(size, 0));                   //PACK(size와 0을 받아서 or연산을 한다.) 해당 값을 현재 bp의 풋터(확장된 size를 기반으로 풋터 위치를 계산)가 가리키는 워드에 저장한다. 
        PUT(HDRP(PREV_BLKP(bp)), PACK(size, 0));        //PACK한 값을 이전 블록의 헤더가 가리키는 워드에 저장한다.
...
{   
    ...
    PUT(heap_listp + (1*WSIZE), PACK(DSIZE, 1));                    //PACK(더블 워드 사이즈와 1(할당됨))값을 한 워드를 지난 포인터의 워드에 대입한다.(프롤로그 헤더)
    PUT(heap_listp + (2*WSIZE), PACK(DSIZE, 1 ));                   //PACK(더블 워드 사이즈와 1(할당됨))값을 두 워드를 지난 포인터의 워드에 대입한다.(프롤로그 풋터)
    PUT(heap_listp + (3*WSIZE), PACK(0, 1));                        //PACK(0의 시이즈와 1(할당됨))값을 세 워드를 지난 포인터의 워드에 대입한다. (에필로그 헤더)
...
static void place(void *bp, size_t asize)               //bp와 asize를 매개변수로 받는다.
{
    ...
    {
        PUT(HDRP(bp), PACK(asize, 1));                  //PACK(할당 할 사이즈와 alloc을 or 연산) 현재 bp의 헤더에 값을 대입 
        PUT(FTRP(bp), PACK(asize, 1));                  //PACK 값을 현재 bp의 풋터에 값을 대입 (새로운 풋터 생성)                  
...
void mm_free(void *bp)                   
{
    ...
    PUT(HDRP(bp), PACK(size, 0));               // 블록 헤더에 (크기, 미할당 상태 0)을 기록한다
    PUT(FTRP(bp), PACK(size, 0));               // 블록 풋터에도 (크기, 미할당 상태 0)을 기록한다
